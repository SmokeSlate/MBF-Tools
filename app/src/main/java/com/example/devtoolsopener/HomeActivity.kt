...
class HomeActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        openGuideButton.setOnClickListener { startActivity(Intent(this, GuideActivity::class.java)) }
        openAdvancedButton.setOnClickListener { startActivity(Intent(this, MainActivity::class.java)) }
    ...
    private fun refreshState() {
        worker.execute {
            ...
            runOnUiThread {
                ...
                homeStatus.text = if (connected) {
                    getString(R.string.home_status_ready)
                } else {
                    getString(R.string.home_status_setup_complete)
                }
                homeDetail.text = if (connected) {
                    getString(R.string.home_detail_connected, connectedDeviceName ?: "")
                } else {
                    getString(
                        R.string.home_detail_not_connected,
                        if (devMode) getString(R.string.status_enabled) else getString(R.string.status_not_detected),
                        if (wireless) getString(R.string.status_enabled) else getString(R.string.status_not_detected)
                    )
                }
    ...
    private fun renderFaq(items: List<FaqItem>) {
        faqContainer.removeAllViews()
        items.forEach { item ->
            faqContainer.addView(createFaqEntryView(item))
        }
    }
